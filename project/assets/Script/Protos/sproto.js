...
var sproto = (function() {
    ...
    function toword(stream, offset = 0) {
        return (stream[offset+0] & 0xff) | (stream[offset+1] & 0xff) << 8;
    };

    function todword(stream, offset = 0) {
        return ((stream[offset+0] & 0xff) | (stream[offset+1] & 0xff) << 8 | (stream[offset+2] & 0xff) << 16 | (stream[offset+3] & 0xff) << 24) >>> 0;
    };

    function count_array(stream, offset = 0) {
        var stream_begin = offset;
        var length = todword(stream, stream_begin);
        var n = 0;
        stream_begin += SIZEOF_LENGTH;
        while (length > 0) {
            var nsz;
            if (length < SIZEOF_LENGTH) {
                return -1;
            }
            nsz = todword(stream, stream_begin);
            nsz += SIZEOF_LENGTH;
            if (nsz > length) {
                return -1;
            }
    ...
    function struct_field(stream, sz, offset = 0) {
        var stream_begin = offset;
        var field_begin, fn, header, i;
        if (sz < SIZEOF_LENGTH) {
            return -1;
        }

        fn = toword(stream, stream_begin);
        header = SIZEOF_HEADER + SIZEOF_FIELD * fn;
        if (sz < header) {
            return -1;
        }

        field_begin = stream_begin + SIZEOF_HEADER;
        sz -= header;
        stream_begin += header;
        for (i = 0; i < fn; i++) {
            var value = toword(stream, field_begin + (i * SIZEOF_FIELD + SIZEOF_HEADER));
            var dsz;
            if (value != 0) {
                continue;
            }
            ...
            dsz = todword(stream, stream_begin);
            if (sz < SIZEOF_LENGTH + dsz) {
                return -1;
            }

            stream_begin += (SIZEOF_LENGTH + dsz);
    ...
    // stream 是arraybuffer
    function import_string(s, stream, offset = 0) {
        var stream_begin = offset;
        var sz = todword(stream, stream_begin);
        var buffer = "";
        stream_begin += SIZEOF_LENGTH;
        for (var i = 0; i < sz; i++) {
            buffer += String.fromCharCode(stream[stream_begin + i]);
        }
        return buffer;
    };
    ...
    function import_field(s, f, stream, offset = 0) {
        var stream_begin = offset;
        var sz, result_begin, fn;
        var array = 0;
        var tag = -1;
        f.tag = -1;
        f.type = -1;
        ...
        sz = todword(stream, stream_begin);
        stream_begin += SIZEOF_LENGTH;
        result_begin = stream_begin + sz;
        fn = struct_field(stream, sz, stream_begin);
        if (fn < 0) return null;

        stream_begin += SIZEOF_HEADER;
        for (var i = 0; i < fn; i++) {
            var value;
            ++tag;
            value = toword(stream, stream_begin + (SIZEOF_FIELD * i));
            if (value & 1 != 0) {
            ...
            if (tag == 0) {
                ...
                f.name = import_string(s, stream, stream_begin + (fn * SIZEOF_FIELD));
                continue;
            }
    ...
    function import_type(s, t, stream, offset = 0) {
        var stream_begin = offset;
        var result_begin, fn, n, maxn, last;
        var sz = todword(stream, stream_begin);
        stream_begin += SIZEOF_LENGTH;
        result_begin = stream_begin + sz;
        fn = struct_field(stream, sz, stream_begin);
        ...
        for (var i = 0; i < fn * SIZEOF_FIELD; i += SIZEOF_FIELD) {
            var v = toword(stream, stream_begin + (SIZEOF_HEADER + i));
            if (v != 0) return null;
        }
        ...
        t.maxn = 0;
        t.f = null;
        stream_begin += (SIZEOF_HEADER + fn * SIZEOF_FIELD);
        t.name = import_string(s, stream, stream_begin);

        if (fn == 1) {
            return result_begin;
        }

        stream_begin += (todword(stream, stream_begin) + SIZEOF_LENGTH);
        n = count_array(stream, stream_begin);
        if (n < 0) {
            return null;
        }
        ...
        for (var i = 0; i < n; i++) {
            ...
            t.f[i] = new Object();
            var f = t.f[i];
            stream_begin = import_field(s, f, stream, stream_begin);
    ...
    */
    function import_protocol(s, p, stream, offset = 0) {
        var stream_begin = offset;
        var result_begin, sz, fn, tag;
        sz = todword(stream, stream_begin);
        stream_begin += SIZEOF_LENGTH;
        result_begin = stream_begin + sz;
        fn = struct_field(stream, sz, stream_begin);
        ...
        for (var i = 0; i < fn; i++, tag++) {
            var value = toword(stream, stream_begin + (SIZEOF_FIELD * i));
            if (value & 1 != 0) {
            ...
            switch (i) {
                case 0:
                    ...
                    p.name = import_string(s, stream, stream_begin + (SIZEOF_FIELD * fn));
                    break;
                case 1:
                    if (value < 0) {
                        return null;
    ...
    function create_from_bundle(s, stream, sz, offset = 0) {
        var stream_begin = offset;
        var content_begin, typedata_begin, protocoldata_begin;
        var fn = struct_field(stream, sz, stream_begin);
        if (fn < 0 || fn > 2)
            return null;
        stream_begin += SIZEOF_HEADER;
        content_begin = stream_begin + (fn * SIZEOF_FIELD);

        for (var i = 0; i < fn; i++) {
            var value = toword(stream, stream_begin + (i * SIZEOF_FIELD));
            if (value != 0) {
                return null;
            }

            var n = count_array(stream, content_begin);
            if (n < 0) {
                return null;
            }

            if (i == 0) {
                typedata_begin = content_begin + SIZEOF_LENGTH;
                s.type_n = n;
                s.type = new Array();
            } else {
                protocoldata_begin = content_begin + SIZEOF_LENGTH;
                s.protocol_n = n;
                s.proto = new Array();
            }
            content_begin += (todword(stream, content_begin) + SIZEOF_LENGTH);
        }

        for (var i = 0; i < s.type_n; i++) {
            s.type[i] = new Object();
            typedata_begin = import_type(s, s.type[i], stream, typedata_begin);
            if (typedata_begin == null) {
                return null;
            }
        }
        ...
        for (var i = 0; i < s.protocol_n; i++) {
            ...
            protocoldata_begin = import_protocol(s, s.proto[i], stream, protocoldata_begin);
            if (protocoldata_begin == null) {
                return null;
            }
        }
    ...
    function decode_array_object(cb, args, stream, sz, offset = 0) {
        var stream_begin = offset;
        var hsz;
        var index = 1;
        while (sz > 0) {
            ...
            hsz = todword(stream, stream_begin);
            stream_begin += SIZEOF_LENGTH;
            sz -= SIZEOF_LENGTH;
            if (hsz > sz) {
                return -1;
    ...
    function decode_array(cb, args, stream, offset = 0) {
        var stream_begin = offset;
        var sz = todword(stream, stream_begin);
        var type = args.type;
        if (sz == 0) {
            ...
            args.value = null;
            args.length = 0;
            cb(args, offset); // 这里不太确定是什么类型，我也加上了。
            return 0;
        }
        ...
        switch (type) {
            case SPROTO_TINTEGER:
                ...
                if (len == 4) {
                    ...
                    for (var i = 0; i < Math.floor(sz / 4); i++) {
                        var value = expand64(todword(stream, stream_begin + (i * 4)));
                        args.index = i + 1;
                        args.value = value;
                        args.length = 8;
                        cb(args);
                    ...
                    for (var i = 0; i < Math.floor(sz / 8); i++) {
                        var low = todword(stream, stream_begin + (i * 8));
                        var hi = todword(stream, stream_begin + (i * 8 + 4));
            ...
            case SPROTO_TBOOLEAN:
                for (var i = 0; i < sz; i++) {
                    var value = stream[stream_begin + i];
                    args.index = i + 1;
                    args.value = value;
                    args.length = 8;
                    cb(args);
            ...
            case SPROTO_TSTRING:
            case SPROTO_TSTRUCT:
                return decode_array_object(cb, args, stream, sz, stream_begin);
            default:
                return -1;
        }
        return 0;
    ...
    t.createNew = function(binsch) {
        ...
        function sproto_decode(st, data, size, cb, ud, offset = 0) {
            let stream_begin = offset;
            var args = new Object();
            var total = size;
            var stream, datastream_begin, fn, tag;
            ...
            if (size < fn * SIZEOF_FIELD)
                return -1;
            datastream_begin = stream_begin + (fn * SIZEOF_FIELD);
            size -= fn * SIZEOF_FIELD;
            args.ud = ud;

            tag = -1;
            for (var i = 0; i < fn; i++) {
                var currentdata_begin = null;
                var f = null;
                var value = toword(stream, stream_begin + (i * SIZEOF_FIELD));
                ++tag;
                if (value & 1 != 0) {
                ...
                value = Math.floor(value / 2) - 1;
                currentdata_begin = datastream_begin;
                if (value < 0) {
                    var sz;
                    if (size < SIZEOF_LENGTH) {
                        return -1;
                    }
                    sz = todword(stream, datastream_begin);
                    if (size < sz + SIZEOF_LENGTH) {
                        return -1;
                    }
                    datastream_begin += (sz + SIZEOF_LENGTH);
                ...
                if (value < 0) {
                    if ((f.type & SPROTO_TARRAY) != 0) {
                        if (decode_array(cb, args, stream, currentdata_begin)) {
                            return -1;
                        }
                    } else {
                        switch (f.type) {
                            case SPROTO_TDOUBLE:
                                {
                                    var sz = todword(stream, currentdata_begin);
                                    if (sz == 8){
                                        let doubleBin = stream.slice(currentdata_begin + SIZEOF_LENGTH, currentdata_begin + SIZEOF_LENGTH+8);
                            ...
                            case SPROTO_TINTEGER:
                                {
                                    var sz = todword(stream, currentdata_begin);
                                    if (sz == 4) {
                                        var v = expand64(todword(stream, currentdata_begin + SIZEOF_LENGTH));
                                        args.value = v;
                                        args.length = 8;
                                        cb(args);
                                    } else if(sz == 8) {
                                        var low = todword(stream, currentdata_begin + SIZEOF_LENGTH);
                                        var hi = todword(stream, currentdata_begin + (SIZEOF_LENGTH + 4));
                            ...
                            case SPROTO_TSTRUCT:
                                {
                                    ...
                                    args.value = stream;
                                    args.length = sz;
                                    if (cb(args, currentdata_begin + SIZEOF_LENGTH) != 0) {
                                        return -1;
                                    }
                                    break;
                                }
                ...
                case SPROTO_TSTRING:
                    {
                        ...
                        for (var i = 0; i < args.length; i++) {
                            arr.push(args.value[offset + i]); // 这里是赋值，所以用了偏移进行优化。
                        }
                        if (args.extra){
                            value = arr;
                        } else {
                ...
                case SPROTO_TSTRUCT:
                    {
                        ...
                        if (args.mainindex >= 0) {
                            ...
                            r = sproto_decode(args.subtype, args.value, args.length, decode, sub, offset);
        ...
        host.dispatch = function(buffer) {
            var sp = this.proto;
            var bin = sp.unpack(buffer); // 解压数据，展开单个32位。
            header_tmp.type = null;
