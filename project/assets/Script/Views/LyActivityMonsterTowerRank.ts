...
export class LyActivityMonsterTowerRank extends ViewLayer {
    ...
    public onViewCreate(params:any):void {
        ...
        label_title.text = StrVal.LYACTIVITY_MONSTERTOWER.STR14;

        // 关闭
        let btn_back: fgui.GButton = uiPanel.getChild("btn_back");
        btn_back.onClick(() => {
            ViewDispatcher.pushViewEvent(null, ViewDispatcher.EVENT_DESTROY, LyActivityMonsterTowerRank, 0, null);
        })
        ...
        let towerItem = LocaleData.getTowerItem(params.selfRank.score > 0 ? params.selfRank.score : 1);
        ...
        label_selfrank.text = UtilsTool.stringFormat(StrVal.LYACTIVITY_MONSTERTOWER.STR202, [params.selfRank.rankOf > 0 ? params.selfRank.rankOf : StrVal.LYCHALLENGE_DUEL.STR9]);
        let label_selfdamage:fgui.GButton = group_main.getChild("label_selfdamage");
        label_selfdamage.text = UtilsTool.stringFormat(StrVal.LYACTIVITY_MONSTERTOWER.STR201, [towerItem.tierID, towerItem.stageID]);

        let duelRanks:Array<ActivityRankPlayer> = params.towerRank;
        ...
        list_rank.itemRenderer = (index:number, child:fgui.GComponent) => {
            ...
            let item = LocaleData.getTowerItem(rankItem.score > 0 ? rankItem.score : 1);
            label_duelScore.text = UtilsTool.stringFormat(StrVal.LYACTIVITY_MONSTERTOWER.STR201, [item.tierID, item.stageID]);
        }
