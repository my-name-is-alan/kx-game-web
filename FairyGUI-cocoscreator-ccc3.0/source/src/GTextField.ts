...
export class GTextField extends GObject {
    ...
    public get stroke(): number {
        return this._label ? (<any>this._label).outlineWidth : 0;
    }
    ...
    public set stroke(value: number) {
        ...
        (<any>this._label).outlineWidth = value;
        (<any>this._label).enableOutline = value > 0;
        if (value > 0)
    ...
    public set shadowOffset(value: Vec2) {
        ...
        if (!this._label)
            return;
        (<any>this._label).shadowOffset = new Vec2(this._shadowOffset.x, -this._shadowOffset.y);
        (<any>this._label).enableShadow = value.x != 0 || value.y != 0;
        if ((<any>this._label).enableShadow)
    ...
    protected updateStrokeColor() {
        if (!this._label || !(<any>this._label).enableOutline)
            return;
        if (!this._strokeColor)
            this._strokeColor = new Color();
        if (this._grayed)
            (<any>this._label).outlineColor = toGrayedColor(this._strokeColor);
        else
            (<any>this._label).outlineColor = this._strokeColor;
    }

    protected updateShadowColor() {
        if (!this._label || !(<any>this._label).enableShadow)
            return;
        if (!this._shadowColor)
            this._shadowColor = new Color();
        if (this._grayed)
            (<any>this._label).shadowColor = toGrayedColor(this._shadowColor);
        else
            (<any>this._label).shadowColor = this._shadowColor;
    }
