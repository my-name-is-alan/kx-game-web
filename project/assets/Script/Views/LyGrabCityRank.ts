...
export class LyGrabCityRank extends ViewLayer {
    public constructor() {
        super();
        this.viewResI.resName = "LyGrabCity";
        this.viewResI.pkgName = "LyGrabCity";
        this.viewResI.comName = "LyGrabCityRank";
    }
    ...
    public onViewCreate(params:any): void {
        ...
        getUiPanel.getChild("btn_back").onClick(()=>{
            ViewDispatcher.pushViewEvent(null, ViewDispatcher.EVENT_DESTROY, LyGrabCityRank, 0, null);
        });
        ...
        let showClanRank = (params)=>{
            let fullInfo = GameServerData.getInstance().getPlayerFullInfo()
            let selfClan = params.selfClan
            let clanList = params.rankList
            ...
            list_clan.itemRenderer = ((index: number, obj: fgui.GButton) => {
                ...
                obj.getChild("label_level").text = ""
                obj.getChild("label_leaderName").text = StrVal.LYGRABCITY.STR39 + clanInfoItem.leaderName
                obj.getChild("label_score").text = params.type == 1 ? StrVal.LYGRABCITY.STR34 + ":" + UtilsTool.nToFStr(clanInfoItem.combatPower) : StrVal.LYGRABCITY.STR33 + ":" + UtilsTool.nToFStr(clanInfoItem.score);
            ...
            label_clanName.text = selfClan.name
            let label_flag: fgui.GLoader = group_main.getChild("label_flag")
            let tt = LocaleData.getClanFlagById(selfClan.flag)
            if (tt) {
            ...
            label_clanRank.text =  selfClan.rankOf > 0 ? UtilsTool.stringFormat(StrVal.LYCLAN.STR18, [selfClan.rankOf]) : StrVal.LYCHALLENGE_DUEL.STR9;
            let label_myClanRankScore: fgui.GLabel = group_main.getChild("label_myClanRankScore")
            label_myClanRankScore.text = params.type == 1? StrVal.LYGRABCITY.STR34 + ":" + UtilsTool.nToFStr(selfClan.combatPower) : StrVal.LYGRABCITY.STR33 + ":" + UtilsTool.nToFStr(selfClan.score); 
        }

        let c1 :fgui.Controller = group_main.getController("c1")
        if (params.type == 1) {
            btn_gr.visible = false
            btn_bp.visible = false
            c1.selectedIndex = 1
            showClanRank(params)
            label_title.text = StrVal.LYGRABCITY.STR40
        }else{
            btn_gr.visible = true
            btn_bp.visible = true
            c1.selectedIndex = 0
            let grabCityPlayer: any = GameServerData.getInstance().getGrabCityPlayer()
            label_title.text = StrVal.LYGRABCITY.STR41
            ...
            if (grabCityPlayer.clanState) {
                let slefInfo = grabCityPlayer.clanState.playerInfo[grabCityPlayer.selfRank -1]
                label_selfrank.text = (slefInfo.rankOf > 0 ? String(slefInfo.rankOf) : StrVal.LYCHALLENGE_DUEL.STR9);
                let label_selfdamage:fgui.GButton = group_main.getChild("label_selfdamage");
                label_selfdamage.text = StrVal.LYGRABCITY.STR32 + ":" + [UtilsTool.nToFStr(slefInfo.harm)];
            }else{
                label_selfrank.text = StrVal.LYCHALLENGE_DUEL.STR9
            }

            let duelRanks = params.rankList;
            ...
            list_rank.itemRenderer = (index:number, child:fgui.GComponent) => {
                ...
                label_duelScore.text = StrVal.LYGRABCITY.STR32 + ":" + UtilsTool.nToFStr(rankItem.harm)
            }
